            ...
            <div className="relative overflow-hidden rounded-3xl bg-linear-to-br from-primary via-primary to-primary-dark text-white shadow-2xl w-full max-w-7xl group">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-stretch">
                    <div className="flex flex-[1.5] flex-col justify-center gap-6 p-8 md:p-12 lg:p-16">
                        <div className="flex items-center gap-3 text-accent-gold font-black tracking-[0.2em] text-xs uppercase bg-white/10 w-fit px-4 py-2 rounded-lg backdrop-blur-md border border-white/10 shadow-sm">
                            <span className="material-symbols-outlined text-[1rem] font-bold">verified</span>
                            {data?.badgeText}
                        </div>
                        <h3 className="text-2xl lg:text-4xl font-black leading-tight font-lexend tracking-tight drop-shadow-md">{data?.title}</h3>
                        <p className="text-blue-50/90 text-sm lg:text-base font-sans leading-relaxed max-w-2xl font-light opacity-95">
                            {data?.description}
                        </p>
                        {data?.primaryButton?.text && (
                            <a
                                href={data.primaryButton.link}
                                className="flex w-fit items-center gap-3 rounded-full bg-white px-6 py-3 text-primary text-sm font-bold hover:bg-gray-100 transition-all shadow-xl hover:shadow-2xl hover:-translate-y-1 group/btn"
                            >
